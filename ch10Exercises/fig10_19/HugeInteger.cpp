...
HugeInteger::HugeInteger(const string& number) {
   // place digits of argument into array
   unsigned int length{number.size()}; //JEH fix unsigned int to cure narrowing conversion

   for (unsigned int j{digits - length}, k{0}; j < digits; ++j, ++k) { //JEH add unsigned here too.
      if (isdigit(number[k])) { // ensure that character is a digit
         integer[j] = number[k] - '0'; // JEH: Hmmm.
      }
   }
}
